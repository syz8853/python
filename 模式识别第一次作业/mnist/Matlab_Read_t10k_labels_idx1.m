...
% 用于读取MNIST数据集中t10k-labels.idx1-ubyte文件并将其转换成txt格式文件输出。
% 用法：运行程序，会弹出选择测试图片数据文件t10k-labels.idx1-ubyte路径的对话框
% 和保存测试图片标签数据文件(txt格式)路径的对话框，选择路径后程序自动运行完毕，期间进度条会显示处理进度。
% 整个程序运行过程需约1分钟时间。
...
%保存训练图片标签数据文件txt格式
[FileName2,PathName2] = uiputfile('测试图片标签数据.txt','保存测试图片标签数据txt文件');
